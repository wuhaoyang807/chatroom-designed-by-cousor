...
class CallDialog(QDialog):    """语音通话对话框"""    call_ended = pyqtSignal()    def __init__(self, parent=None, friend_name=None, is_caller=False, udp_thread=None, target_addr=None,                 username=None):        super().__init__(parent)        self.friend_name = friend_name        self.is_caller = is_caller        self.udp_thread = udp_thread        self.target_addr = target_addr        self.username = username        self.audio_recorder = None        self.audio_player = None        self.call_active = False        self.error_occurred = False        self.audio_devices = None        self.init_ui()        if is_caller:            self.status_label.setText(f"正在等待 {friend_name} 接听...")        else:            self.status_label.setText(f"与 {friend_name} 通话中...")            # 延迟获取音频设备，避免阻塞UI            QTimer.singleShot(500, self.delayed_start_call)    def delayed_start_call(self):        """延迟启动通话，避免阻塞UI"""        try:            # 获取音频设备            self.audio_devices = self.get_audio_devices()            if self.audio_devices['input'] is not None and self.audio_devices['output'] is not None:                self.start_call()            else:                QMessageBox.warning(self, '错误', '音频设备获取失败，无法开始通话')                self.end_call()        except Exception as e:            logging.error(f"延迟启动通话失败: {e}")            QMessageBox.warning(self, '错误', f'启动通话失败: {e}')            self.end_call()
